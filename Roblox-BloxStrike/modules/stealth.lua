...
-- AC reads the real instance property, so we swap values temporarily
-- This avoids breaking Roblox UI which also reads properties

local spoofRegistry = {}  -- {instance = {prop = {real=, fake=}}}

-- Get the real value of a property (before any spoofing)
function Stealth.getReal(instance, prop)
    if spoofRegistry[instance] and spoofRegistry[instance][prop] then
        return spoofRegistry[instance][prop].Real
    end
    return instance[prop]
end

-- Set the real value (bypassing spoof tracking)
function Stealth.setReal(instance, prop, value)
    instance[prop] = value
end

-- Register a spoof: stores the real value and temporarily writes the fake value
-- AC reads the fake value directly from the instance property
function Stealth.spoofProperty(instance, prop, fakeValue)
    if not Flags.StealthSpoofAll then return end
    if not instance or not prop then return end

    -- If already spoofed, just update the fake value
    if spoofRegistry[instance] and spoofRegistry[instance][prop] then
        spoofRegistry[instance][prop].Fake = fakeValue
        -- Write fake value directly to instance
        instance[prop] = fakeValue
        return
    end

    -- First time spoofing: save real value
    spoofRegistry[instance] = spoofRegistry[instance] or {}
    spoofRegistry[instance][prop] = {
        Real = instance[prop],
        Fake = fakeValue,
    }
    -- Write fake value directly to instance (AC reads this)
    instance[prop] = fakeValue
end

-- Remove spoof: restore real value to instance
function Stealth.unspoofProperty(instance, prop)
    if spoofRegistry[instance] and spoofRegistry[instance][prop] then
        local entry = spoofRegistry[instance][prop]
        instance[prop] = entry.Real
        spoofRegistry[instance][prop] = nil
    end
end

-- Remove all spoofs for an instance
function Stealth.unspoofAll(instance)
    if spoofRegistry[instance] then
        for prop, entry in pairs(spoofRegistry[instance]) do
            instance[prop] = entry.Real
        end
        spoofRegistry[instance] = nil
    end
end

-- Update fake value without re-saving real
function Stealth.updateFake(instance, prop, fakeValue)
    if spoofRegistry[instance] and spoofRegistry[instance][prop] then
        spoofRegistry[instance][prop].Fake = fakeValue
        instance[prop] = fakeValue
    end
...
-- Check if a property is currently spoofed
function Stealth.isSpoofed(instance, prop)
    return spoofRegistry[instance] and spoofRegistry[instance][prop] ~= nil
end
...
task.spawn(function()
    while true do
        task.wait(0.5)
        pcall(function()
            if not Flags.StealthSpoofAll then
                -- When spoofing is disabled, restore all values
                for instance, props in pairs(spoofRegistry) do
                    for prop, entry in pairs(props) do
                        pcall(function() instance[prop] = entry.Real end)
                    end
                end
                spoofRegistry = {}
                continue
            ...
            -- === WALKSPEED SPOOFING ===
            if Flags.StealthSpoofSpeed and h and Flags.SpeedBoost then
                local targetSpeed = Flags.SpeedValue or 20
                -- Set the actual modified speed
                h.WalkSpeed = targetSpeed
                -- Immediately spoof it back to 16 for AC
                Stealth.spoofProperty(h, "WalkSpeed", 16)
            elseif Flags.StealthSpoofSpeed and h and not Flags.SpeedBoost then
                if Stealth.isSpoofed(h, "WalkSpeed") then
                    Stealth.unspoofProperty(h, "WalkSpeed")
                end
            end

            -- === JUMPPOWER SPOOFING ===
            if Flags.StealthSpoofJump and h and (Flags.Bhop or Flags.JumpPower) then
                local realJump = 70  -- actual jump power we want
                h.JumpPower = realJump
                Stealth.spoofProperty(h, "JumpPower", 50)  -- fake: default 50
            elseif Flags.StealthSpoofJump and h then
                if Stealth.isSpoofed(h, "JumpPower") then
                    Stealth.unspoofProperty(h, "JumpPower")
                end
            end

            -- === HIPHEIGHT SPOOFING ===
            if Flags.StealthSpoofHip and h and (Flags.ThirdPerson or Flags.NoClip) then
                h.HipHeight = 2
                Stealth.spoofProperty(h, "HipHeight", 0)  -- fake: default 0
            elseif Flags.StealthSpoofHip and h then
                if Stealth.isSpoofed(h, "HipHeight") then
                    Stealth.unspoofProperty(h, "HipHeight")
                end
            end

            -- === FOV SPOOFING ===
            if Flags.StealthSpoofFOV and cam and (Flags.FOVChanger or Flags.VMFOV) then
                local targetFOV = Flags.FOVValue or 90
                cam.FieldOfView = targetFOV
                Stealth.spoofProperty(cam, "FieldOfView", 70)  -- fake: default 70
            elseif Flags.StealthSpoofFOV and cam then
                if Stealth.isSpoofed(cam, "FieldOfView") then
                    Stealth.unspoofProperty(cam, "FieldOfView")
                end
            end
        end)
    end
...
-- Expose spoofing API for other modules
BS.PropertySpoof = {
    Spoof = function(instance, prop, fake)
        Stealth.spoofProperty(instance, prop, fake)
    end,
    Unspoof = function(instance, prop)
        Stealth.unspoofProperty(instance, prop)
    end,
    GetReal = function(instance, prop)
        return Stealth.getReal(instance, prop)
    end,
    SetReal = function(instance, prop, value)
        Stealth.setReal(instance, prop, value)
    end,
    IsSpoofed = function(instance, prop)
        return Stealth.isSpoofed(instance, prop)
    end,
    UpdateFake = function(instance, prop, fake)
        Stealth.updateFake(instance, prop, fake)
    end,
}
...
task.spawn(function()
 ...
 -- Anti Debug Engine (fixed: originals saved once to prevent infinite wrapping)
local _origDebugGetInfo = debug and debug.getinfo
local _origDebugTraceback = debug and debug.traceback
task.spawn(function()
    while true do task.wait(5)
        if Flags.StealthAntiDebug then
            pcall(function()
                if debug and _origDebugGetInfo then
                    debug.getinfo=function(level,what)
                        local info=_origDebugGetInfo(level,what)
                ...
                if debug and _origDebugTraceback then
                    debug.traceback=function(level)
                        local tb=_origDebugTraceback(level)
                        if tb then
            ...
            pcall(function()
                if debug and _origDebugGetInfo then debug.getinfo = _origDebugGetInfo end
                if debug and _origDebugTraceback then debug.traceback = _origDebugTraceback end
            end)
        end
    end
end)
...
function Stealth.selfHeal()
    ...
    pcall(function()
        ...
        -- Re-apply debug overrides (use saved originals, not current state)
        if Flags.StealthAntiDebug and debug and _origDebugGetInfo then
            debug.getinfo = function(level, what)
                local info = _origDebugGetInfo(level, what)
                if info and info.source and info.source:find("BloxStrike") then
                    info.source = "[C]"
...
-- ═══════════════════════════════════════════════════════════════
-- ANTI-SCREENSHOT (SAFE: only prints, no SetCore hook)
-- ═══════════════════════════════════════════════════════════════
BS.AntiScreenshot = {Active = false}

function BS.AntiScreenshot:Activate()
    self.Active = true
    -- Only hide on PrintScreen key (no SetCore hook to avoid UI breakage)
    pcall(function()
        UIS.InputBegan:Connect(function(input, gpe)
            if gpe then return end
            if input.KeyCode == Enum.KeyCode.PrintScreen and self.Active then
...
function BS.AntiScreenshot:Deactivate()
    self.Active = false
end
...
task.spawn(function()
    while true do
        task.wait(15)
        pcall(function()
            if Flags.StealthAntiDetect then
