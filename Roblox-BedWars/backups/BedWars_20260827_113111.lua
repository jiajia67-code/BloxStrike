...
-- ═══════════════════════════════════════════════════════════════
    ...
    page:Toggle("Sprint", false, function(v)
        Flags.Sprint = v
        if v then
    ...
    -- ═══ Kill Aura ═══
    page:Toggle("Kill Aura", false, function(v) Flags.KA = v end, "Combat", "Auto attack nearest enemy")
    page:Slider("KA Range", 5, 20, 18, function(v) Flags.KA_Range = v end, "Combat")
    page:Slider("KA Speed", 1, 20, 12, function(v) Flags.KA_Speed = v end, "Combat")
    page:Toggle("KA Auto Swing", true, function(v) Flags.KA_Swing = v end, "Combat")
    page:Toggle("KA Rotate", false, function(v) Flags.KA_Rotate = v end, "Combat", "Rotate to face target")
    page:Toggle("KA Multi", false, function(v) Flags.KA_Multi = v end, "Combat", "Attack multiple targets")
    ...
    -- ═══ Crystal Aura ═══
    page:Toggle("Crystal Aura", false, function(v) Flags.CA = v end, "Combat", "Auto place & break crystals")
    page:Slider("CA Range", 3, 15, 8, function(v) Flags.CA_Range = v end, "Combat")
    page:Slider("CA Delay", 0, 200, 50, function(v) Flags.CA_Delay = v / 1000 end, "Combat")
    page:Toggle("CA Auto Break", true, function(v) Flags.CA_Break = v end, "Combat")
    ...
    -- ═══ Auto Clicker ═══
    page:Toggle("Auto Click", false, function(v) Flags.AutoClick = v end, "Combat", "Auto click attack")
    page:Slider("Click CPS", 5, 20, 14, function(v) Flags.ClickCPS = v end, "Combat")
    ...
    -- ═══ Silent Aim ═══
    page:Toggle("Silent Aim", false, function(v) Flags.SilentAim = v end, "Combat", "Silent aimbot (no visual)")
    page:Slider("SA FOV", 10, 180, 90, function(v) Flags.SA_FOV = v end, "Combat")
    page:Toggle("SA Headshot", false, function(v) Flags.SA_Head = v end, "Combat")
    page:Toggle("SA Wall Check", true, function(v) Flags.SA_Wall = v end, "Combat")

    -- ═══ Trigger Bot ═══
    page:Toggle("Trigger Bot", false, function(v) Flags.TriggerBot = v end, "Combat", "Auto shoot when aimed at enemy")
    ...
    -- ═══ Auto Release (Bow) ═══
    page:Toggle("Auto Release", false, function(v) Flags.AutoRelease = v end, "Combat", "Auto release bow")
    page:Slider("Release %", 10, 100, 80, function(v) Flags.ReleasePercent = v end, "Combat")

    -- ═══ Projectile Aimbot ═══
    page:Toggle("Proj Aimbot", false, function(v) Flags.ProjAim = v end, "Combat", "Predictive aim for projectiles")
    page:Slider("Proj FOV", 10, 180, 60, function(v) Flags.ProjFOV = v end, "Combat")

    -- ═══ AimAssist ═══
    page:Toggle("AimAssist", false, function(v) Flags.AimAssist = v end, "Combat", "Smooth mouse aim assist")
    page:Slider("AA Smooth", 1, 20, 8, function(v) Flags.AA_Smooth = v end, "Combat")
    page:Slider("AA FOV", 10, 180, 60, function(v) Flags.AA_FOV = v end, "Combat")
    page:Toggle("AA Team Check", true, function(v) Flags.AA_TeamCheck = v end, "Combat")
    page:Toggle("AA Wall Check", true, function(v) Flags.AA_WallCheck = v end, "Combat")

    task.spawn(function()
        while task.wait() do
            if Flags.AimAssist and BW.alive() then
                local mouse = UserInputService:GetMouseLocation()
                local cam = workspace.CurrentCamera
                local myHRP = BW.hrp()
                if not myHRP then continue end

                local bestTarget, bestDist = nil, Flags.AA_FOV or 60
                for _, e in pairs(BW.enemies()) do
                    local eHRP = e.HRP
                    if eHRP then
                        local pos, onScreen = cam:WorldToViewportPoint(eHRP.Position)
                        if onScreen then
                            local dist = (Vector2.new(pos.X, pos.Y) - mouse).Magnitude
                            if dist < bestDist then
                                if not (Flags.AA_WallCheck and not BW.hasLineOfSight(myHRP.Position, eHRP.Position)) then
                                    bestDist = dist
                                    bestTarget = e
                                end
                            end
                ...
                if bestTarget then
                    local eHRP = bestTarget.HRP
                    local pos = cam:WorldToViewportPoint(eHRP.Position)
                    local target = Vector2.new(pos.X, pos.Y)
                    local delta = (target - mouse) / (Flags.AA_Smooth or 8)
    ...
    -- ═══ NoClickDelay ═══
    page:Toggle("No Click Delay", false, function(v) Flags.NoClickDelay = v end, "Combat", "Remove attack cooldown")
    task.spawn(function()
        while true do
            if Flags.NoClickDelay then
                pcall(function()
                    local character = BW.char()
                    if character then
    ...
    page:Toggle("Proj Aimbot", false, function(v) Flags.ProjAim = v end, "Combat", "Predictive aim for projectiles")
    page:Slider("Proj FOV", 10, 180, 60, function(v) Flags.ProjFOV = v end, "Combat")
    page:Toggle("Proj Prediction", true, function(v) Flags.ProjPredict = v end, "Combat")

    -- ═══ Velocity (Anti-KB) ═══
    page:Toggle("Velocity", false, function(v) Flags.Velocity = v end, "Combat", "Anti-knockback")
    page:Slider("Vel X", 0, 100, 0, function(v) Flags.VelX = v end, "Combat")
    page:Slider("Vel Y", 0, 100, 0, function(v) Flags.VelY = v end, "Combat")

    task.spawn(function()
        while true do
            if Flags.Velocity and BW.alive() then
                local hrp = BW.hrp()
                if hrp then
                    local vel = hrp.AssemblyLinearVelocity
                    local newX = vel.X * ((Flags.VelX or 0) / 100)
                    local newZ = vel.Z * ((Flags.VelX or 0) / 100)
                    local newY = vel.Y * ((Flags.VelY or 0) / 100)
    ...
    -- ═══ Reach Extend ═══
    page:Toggle("Reach", false, function(v) Flags.Reach = v end, "Combat", "Extend attack range")
    page:Slider("Reach Dist", 2, 10, 4, function(v) Flags.ReachDist = v end, "Combat")

    -- ═══ Auto Sword ═══
    page:Toggle("Auto Sword", false, function(v) Flags.AutoSword = v end, "Combat", "Auto equip best sword")
    page:Slider("Sword Delay", 100, 1000, 300, function(v) Flags.SwordDelay = v / 1000 end, "Combat")

    task.spawn(function()
        while true do
            if Flags.AutoSword and BW.alive() then
                local swordNames = {"emerald_sword", "diamond_sword", "iron_sword", "stone_sword", "wood_sword"}
                local char = BW.char()
                if char then
                    ...
                    if not hasSword then
                        for _, name in pairs(swordNames) do
                            if BW.findTool(name) then
                                BW.equipTool(name)
                                break
                            end
                        end
            ...
            task.wait(Flags.SwordDelay or 0.3)
        end
    end)

    -- ═══ No Fall ═══
    page:Toggle("No Fall", false, function(v) Flags.NoFall = v end, "Combat", "Prevent fall damage")
    task.spawn(function()
        while true do
            if Flags.NoFall and BW.alive() then
                local char = BW.char()
                if char then
    ...
    -- ═══ Target Strafe ═══
    page:Toggle("Target Strafe", false, function(v) Flags.TargetStrafe = v end, "Combat", "Circle strafe around target")
    page:Slider("Strafe Speed", 1, 10, 5, function(v) Flags.StrafeSpeed = v end, "Combat")
    page:Slider("Strafe Radius", 2, 10, 4, function(v) Flags.StrafeRadius = v end, "Combat")

    task.spawn(function()
        local angle = 0
        while true do
            if Flags.TargetStrafe and BW.alive() then
                local target, dist = BW.nearestEnemy(20)
                if target and dist then
                    local hrp = BW.hrp()
                    local eHRP = target.HRP
                    if hrp and eHRP then
                        angle = angle + (Flags.StrafeSpeed or 5) * 0.05
                        local radius = Flags.StrafeRadius or 4
                        local offset = Vector3.new(math.cos(angle) * radius, 0, math.sin(angle) * radius)
                        local targetPos = eHRP.Position + offset
                        hrp.CFrame = CFrame.new(hrp.Position, Vector3.new(targetPos.X, hrp.Position.Y, targetPos.Z))
                        hrp.Velocity = (targetPos - hrp.Position).Unit * 20
                    end
                end
            end
            task.wait()
    ...
    -- ═══ Hit Boxes ═══
    page:Toggle("Hit Boxes", false, function(v) Flags.HitBoxes = v end, "Combat", "Expand enemy hitboxes")
    page:Slider("HB Size", 1, 10, 3, function(v) Flags.HBSize = v end, "Combat")
    ...
    task.spawn(function()
        while true do
            if Flags.HitBoxes then
                for _, e in pairs(BW.enemies()) do
                    if e.Char then
                        local hrp = e.Char:FindFirstChild("HumanoidRootPart")
                        if hrp then
                            pcall(function()
                                hrp.Size = Vector3.new(Flags.HBSize or 3, Flags.HBSize or 3, Flags.HBSize or 3)
                                hrp.Transparency = 0.7
                                hrp.CanCollide = false

    -- AimAssist
    page:Toggle("AimAssist", false, function(v) Flags.AimAssist = v end)
    page:Slider("AA Smooth", 1, 20, 8, function(v) Flags.AA_Smooth = v end)
    page:Slider("AA FOV", 10, 180, 60, function(v) Flags.AA_FOV = v end)
    page:Toggle("AA Wall Check", true, function(v) Flags.AA_WallCheck = v end)
    ...
    task.spawn(function()
        while task.wait() do
            if Flags.AimAssist and BW.alive() then
                ...
                if bestTarget then
                    local pos = cam:WorldToViewportPoint(bestTarget.HRP.Position)
                    local target = Vector2.new(pos.X, pos.Y)
                    local delta = (target - mouse) / (Flags.AA_Smooth or 8)
                    pcall(function() mousemoverel(delta.X, delta.Y) end)
                end
    ...
    -- Velocity (Anti-KB)
    page:Toggle("Velocity", false, function(v) Flags.Velocity = v end)
    ...
    task.spawn(function()
        while true do
            if Flags.Velocity and BW.alive() then
                local hrp = BW.hrp()
                if hrp then
                    local str = (Flags.VelStr or 0) / 100
                    local vel = hrp.AssemblyLinearVelocity
                    hrp.AssemblyLinearVelocity = Vector3.new(vel.X * str, vel.Y, vel.Z * str)
                end
            end
            task.wait()
        end
    ...
    -- Auto Sword
    page:Toggle("Auto Sword", false, function(v) Flags.AutoSword = v end)
    task.spawn(function()
        while true do
            if Flags.AutoSword and BW.alive() then
                ...
                if char then
                    local hasSword = false
                    for _, tool in pairs(char:GetChildren()) do
                        if tool:IsA("Tool") and tool.Name:find("sword") then hasSword = true; break end
                    end
                    if not hasSword then
                        for _, name in pairs(swords) do
                            if BW.findTool(name) then BW.equipTool(name); break end
                        end
                    end
                end
            end
    ...
    task.spawn(function()
        while true do
            if Flags.NoFall and BW.alive() then
                local h = BW.hum()
                if h then pcall(function() h:ChangeState(Enum.HumanoidStateType.Freefall) end) end
            end
            task.wait(0.1)
        end
    end)
    ...
    -- Hit Boxes
    page:Toggle("Hit Boxes", false, function(v) Flags.HitBoxes = v end)
    ...
    -- Target Strafe
    page:Toggle("Target Strafe", false, function(v) Flags.TargetStrafe = v end)
    page:Slider("Strafe Speed", 1, 10, 5, function(v) Flags.StrafeSpeed = v end)
    local strafeAngle = 0
    task.spawn(function()
        while true do
            if Flags.TargetStrafe and BW.alive() then
                ...
                if target and dist then
                    local hrp = BW.hrp()
                    if hrp then
                        strafeAngle = strafeAngle + (Flags.StrafeSpeed or 5) * 0.05
                        local offset = Vector3.new(math.cos(strafeAngle) * 4, 0, math.sin(strafeAngle) * 4)
                        local pos = target.HRP.Position + offset
...
-- ═══════════════════════════════════════════════════════════════
    ...
    page:Toggle("Scaffold", false, function(v) Flags.Scaffold = v end, "World", "Auto place blocks under you")
    page:Toggle("Scaffold Down", false, function(v) Flags.ScaffoldDown = v end, "World", "Place blocks downward")
    page:Slider("Scaffold Width", 1, 3, 1, function(v) Flags.ScaffoldWidth = v end, "World")
    page:Toggle("Scaffold Tower", false, function(v) Flags.ScaffoldTower = v end, "World", "Tower mode (hold space)")
    page:Toggle("Scaffold Safe", true, function(v) Flags.ScaffoldSafe = v end, "World", "Extra safety checks")
    ...
    -- ═══ Bed Protector ═══
    page:Toggle("Bed Protect", false, function(v) Flags.BedProtect = v end, "World", "Auto place blocks around bed")
    page:Slider("BP Layers", 1, 3, 2, function(v) Flags.BPLayers = v end, "World")
    ...
    -- ═══ Auto Collect ═══
    page:Toggle("Auto Collect", false, function(v) Flags.AutoCollect = v end, "World", "Auto collect resources")
    page:Slider("Collect Range", 5, 50, 15, function(v) Flags.CollectRange = v end, "World")
    page:Toggle("Collect Iron", true, function(v) Flags.CollectIron = v end, "World")
    page:Toggle("Collect Gold", true, function(v) Flags.CollectGold = v end, "World")
    page:Toggle("Collect Diamond", true, function(v) Flags.CollectDiamond = v end, "World")
    page:Toggle("Collect Emerald", true, function(v) Flags.CollectEmerald = v end, "World")
    ...
    -- ═══ Auto Break Bed ═══
    page:Toggle("Auto Break Bed", false, function(v) Flags.AutoBreakBed = v end, "World", "Auto break enemy beds")
    ...
    -- ═══ Nuker ═══
    page:Toggle("Nuker", false, function(v) Flags.Nuker = v end, "World", "Auto break blocks around you")
    page:Slider("Nuker Range", 3, 10, 5, function(v) Flags.NukerRange = v end, "World")
    ...
    -- ═══ Chest Steal ═══
    page:Toggle("Chest Steal", false, function(v) Flags.ChestSteal = v end, "World", "Auto take items from chests")

    -- ═══ Safe Walk ═══
    page:Toggle("Safe Walk", false, function(v) Flags.SafeWalk = v end, "World", "Prevent falling off edges")

    -- ═══ Xray ═══
    page:Toggle("Xray", false, function(v) Flags.Xray = v end, "World", "See ores through walls")
...
-- ═══════════════════════════════════════════════════════════════
    ...
    page:Toggle("Auto Buy Blocks", false, function(v) Flags.AutoBuyBlocks = v end, "Shop", "Auto buy blocks")
    page:Toggle("Auto Buy Sword", false, function(v) Flags.AutoBuySword = v end, "Shop", "Auto buy best sword")
    page:Toggle("Auto Buy Armor", false, function(v) Flags.AutoBuyArmor = v end, "Shop", "Auto buy best armor")
    page:Toggle("Auto Buy Tools", false, function(v) Flags.AutoBuyTools = v end, "Shop", "Auto buy best tools")
    page:Toggle("Auto Buy Projectiles", false, function(v) Flags.AutoBuyProj = v end, "Shop", "Auto buy projectiles")
    page:Slider("Buy Delay", 100, 2000, 500, function(v) Flags.BuyDelay = v end, "Shop")

    -- ═══ Shop Tier Bypass ═══
    page:Toggle("Tier Bypass", false, function(v) Flags.ShopTierBypass = v end, "Shop", "Access higher shop tiers")
