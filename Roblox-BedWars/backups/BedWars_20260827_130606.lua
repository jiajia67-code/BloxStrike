...
--!nocheck
-- ═══════════════════════════════════════════════════════════════
-- BEDWARS API MODULE v5.1 — Full Real Game API
-- Based on VapeV4 & CatVape source code analysis
-- ═══════════════════════════════════════════════════════════════
...
if KnitInit and Knit then
    ...
    api.Knit = Knit
    print("[API] Knit loaded")
else
    print("[API] Knit not available")
end
...
-- ═══ 5. bedwars Table (Auto-Load Controllers) ═══
api.bw = {}
if Knit then
    api.bw = setmetatable({
        Client = Client,
...
if Knit then
    ...
    print("[API] Controllers (" .. #controllerNames .. "):")
    for _, name in pairs(controllerNames) do
        print("  - " .. name)
    end
end
...
if Client then
    ...
    print("[API] Client remotes: " .. count)
end
if api.NetManaged then
    local count = #api.NetManaged:GetChildren()
    print("[API] _NetManaged remotes: " .. count)
end
...
function api.isBedAlive(bed)
    return bed and not bed:GetAttribute("NoBreak")
end
...
function api.getIronDrops(maxDist)
    local drops = {}
    for _, drop in pairs(CollectionService:GetTagged("ItemDrop")) do
        local dropType = drop:GetAttribute("itemType") or drop.Name
...
function api.getToolCount(name)
    local count = 0
    local c = lplr.Character
    if c then
        for _, tool in pairs(c:GetChildren()) do
            if tool:IsA("Tool") and tool.Name:find(name) then
                count = count + 1
            end
        end
    end
...
function api.getResources()
    ...
    if api.Store then
        pcall(function()
            ...
            if state.Consumable and state.Consumable.inventory then
                for _, item in pairs(state.Consumable.inventory) do
                    if item.consumable == "iron" then res.iron = res.iron + (item.amount or 1)
                    elseif item.consumable == "gold" then res.gold = res.gold + (item.amount or 1)
                    elseif item.consumable == "diamond" then res.diamond = res.diamond + (item.amount or 1)
                    elseif item.consumable == "emerald" then res.emerald = res.emerald + (item.amount or 1)
                    end
                end
            end
        end)
...
-- ═══════════════════════════════════════════════════════════════
-- CONTROLLER SHORTCUTS (Real BedWars APIs)
-- ═══════════════════════════════════════════════════════════════
...
function api.getBlockAt(position)
    local success, result = pcall(function()
        return api.bw.BlockController:getBlockAt(position)
    end)
    return success and result or nil
end
...
-- ═══ Kit-Specific Remotes ═══

-- Adetunde Kit
function api.upgradeAdetunde(upgradeType)
    -- upgradeType: "shield", "speed", "strength"
    return api.invokeNetManaged("UpgradeFrostyHammer", upgradeType)
end

-- Ember Kit
function api.useEmberSaber(weapon, chargeTime)
    api.fireNetManaged("HellBladeRelease", {
        weapon = weapon,
        player = lplr,
        chargeTime = chargeTime or 0.9
    })
end
...
-- Void Hunter Kit
function api.useVoidHunterMark(direction)
    api.fireNetManaged("VoidHunter_MarkAbilityRequest", {
        direction = direction
    })
end
...
function api.getGameTime()
    ...
    return state.GameTime or 0
end

function api.getMatchState()
    local state = api.getBedwarsState()
    return state.MatchState or "Unknown"
end

function api.getPlayerLevel()
    local state = api.getBedwarsState()
    return state.playerLevel or 0
end

print("[API] Loaded (Knit=" .. tostring(Knit ~= nil)
    .. ", Client=" .. tostring(Client ~= nil)
...
print("[Combat] Module loaded (34 features)")
...
-- ═══ 31. Phase (walk through walls) ═══
page:Toggle("Phase", false, function(v) Flags.Phase = v end)
task.spawn(function()
    while task.wait(0.1) do
        if Flags.Phase and alive() then
            local h = lplr.Character:FindFirstChildOfClass("Humanoid")
            if h then h.WalkSpeed = 20 end
        end
    end
...
print("[World] Module loaded (31 features)")
