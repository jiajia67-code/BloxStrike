...
local function getLookingAt()
    ...
    if name:find("wool") or name:find("stone") or name:find("obsidian") or name:find("end_stone") then return "block", target end
...
task.spawn(function()
    while task.wait(0.1) do
        if Flags.AutoTool and alive() then
            ...
            elseif lookingAt == "bed" or lookingAt == "block" or lookingAt == "chest" then
                -- Block/bed/chest: equip best pickaxe
                if Flags.ToolPickPri then bestTool = findBestTool(PICKAXE_TIER) end
                if not bestTool and Flags.ToolPickPri then bestTool = findBestTool(AXE_TIER) end
