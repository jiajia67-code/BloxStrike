...
--!nocheck
-- ═══════════════════════════════════════════════════════════════
-- BEDWARS API MODULE v5.1 — Real Game API Access
-- Based on VapeV4 & CatVape patterns
-- ═══════════════════════════════════════════════════════════════
...
if KnitInit and Knit then
    -- Wait for Knit to be ready
    pcall(function()
        repeat task.wait() until debug.getupvalue(Knit.Start, 1)
    end)
    api.Knit = Knit
    print("[API] Knit framework loaded")
else
    print("[API] Knit not available, using fallback")
end
...
-- ═══ 3. bedwars Table (Auto-Load Controllers) ═══
api.bw = {}
if Knit then
    api.bw = setmetatable({
        Client = Client,
...
if Knit then
    ...
    print("[API] Available controllers (" .. #controllerNames .. "):")
    for _, name in pairs(controllerNames) do
        print("  - " .. name)
    end
end

-- ═══ 6. Dump Available Remotes ═══
if Client then
    print("[API] Dumping remotes...")
    local remoteCount = 0
    ...
    print("[API] Found " .. remoteCount .. " remotes")
end
...
function api.getNearestEnemy(maxDist)
    ...
    for _, p in pairs(api.getEnemies()) do
        if p.Character then
    ...
    if api.Store then
        pcall(function()
            ...
            if state.Consumable and state.Consumable.inventory then
                items = state.Consumable.inventory
            end
        end)
    end
    return items
...
function api.placeBlock(blockType, position)
    pcall(function()
        api.bw.BlockController:placeBlock(blockType, position, Vector3.new(0, 1, 0))
    end)
end

function api.buyItem(itemType)
    pcall(function()
        api.bw.ShopController:purchaseItem(itemType, "main")
    end)
end

print("[API] Module loaded (Knit=" .. tostring(Knit ~= nil) .. ", Client=" .. tostring(Client ~= nil) .. ")")
return api
