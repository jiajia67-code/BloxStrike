...
function BW.getInventory()
    ...
    local emerald = 0
    local blocks = 0
    -- Simple inventory count (placeholder for actual game API)
    return {iron=iron, gold=gold, diamond=diamond, emerald=emerald, blocks=blocks}, iron
end

function BW.buyItem(itemType)
    -- Placeholder for shop API
    pcall(function()
...
function BW.SendWebhook(title, desc, color)
    if not BW.WebhookURL or BW.WebhookURL == "" then return end
    pcall(function()
        ...
        local data = {
            embeds = {{
                ...
                footer = {text = "BedWars Ultimate v5.1"}
            }}
        }
...
-- ═══ Config Save/Load ═══
BW.StarterGui = game:GetService("StarterGui")

function BW.SaveConfig()
    pcall(function()
        local HttpService = game:GetService("HttpService")
        local config = {
            DiscordWebhook = BW.Config.DiscordWebhook or "",
            Flags = {}
        }
