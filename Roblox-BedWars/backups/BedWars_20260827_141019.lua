...
task.spawn(function()
    while task.wait() do
        ...
        task.wait((Flags.CA_Delay or 0.03) + (Flags.CA_Cycle and (1 / (Flags.CA_CycleSpeed or 5)) or 0))
    end
end)
...
task.spawn(function()
    while task.wait() do
        ...
        task.wait((Flags.PotDelay or 500) / 1000)
    end
end)
    ...
    while task.wait() do
        if Flags.JitterClick and alive() then
            pcall(function()
                ...
                if tool then
                    tool:Activate()
                    task.wait(1 / (Flags.JitterCPS or 18))
                    tool:Deactivate()
                    task.wait(0.01)
                end
            end)
...
page:Toggle("Auto Pearl", false, function(v) Flags.CombatPearl = v end)
...
local function invokeRemote(name, ...)
    pcall(function()
        ...
        pcall(function()
            ...
            if remote then remote:CallServer(unpack({...})) end
        end)
