...
--!nocheck
-- ══════════════════════════════════════════════════════════════
-- UI LIBRARY MODULE v6.0 (CatVape-style + Full Mobile)
-- ══════════════════════════════════════════════════════════════
...
function Library:New(config)
    ...
    self.Flags = BW.Flags
    self.Config = config or {Title = "BedWars", Sub = "v6.0"}
    ...
    if BW.isMobile then
        main.Size = UDim2.new(0.94, 0, 0.7, 0)
        main.Position = UDim2.new(0.03, 0, 0.15, 0)
    else
        main.Size = UDim2.new(0, math.floor(580 * scale), 0, math.floor(420 * scale))
        main.Position = UDim2.new(0.5, math.floor(-290 * scale), 0.5, math.floor(-210 * scale))
    end
    ...
    titleBar.Size = UDim2.new(1, 0, 0, ifelse(BW.isMobile, 48, 44))
    ...
    title.TextSize = ifelse(BW.isMobile, 13, 15)
    ...
    hint.Size = UDim2.new(0, 140, 1, 0)
    hint.Position = UDim2.new(1, -190, 0, 0)
    hint.BackgroundTransparency = 1
    hint.Text = ifelse(BW.isMobile, "📱 Swipe tabs", "RightAlt to toggle")
    ...
    local closeSize = ifelse(BW.isMobile, 36, 28)
    local closeBtn = Instance.new("TextButton")
    closeBtn.Size = UDim2.new(0, closeSize, 0, closeSize)
    closeBtn.Position = UDim2.new(1, -(closeSize + 10), 0.5, -(closeSize / 2))
    closeBtn.BackgroundColor3 = Colors.Red
    closeBtn.BorderSizePixel = 0
    closeBtn.Text = "✕"
    closeBtn.TextColor3 = Color3.new(1, 1, 1)
    closeBtn.TextSize = ifelse(BW.isMobile, 16, 12)
    ...
    if BW.isMobile then
        ...
        fab.Parent = sg
        addCorner(fab, 28)
        addStroke(fab, Color3.new(1, 1, 1), 2)

        -- FAB shadow
        local fabShadow = Instance.new("ImageLabel")
        fabShadow.Size = UDim2.new(1, 40, 1, 40)
        fabShadow.Position = UDim2.new(0.5, 0, 0.5, 4)
        fabShadow.AnchorPoint = Vector2.new(0.5, 0.5)
        fabShadow.BackgroundTransparency = 1
        fabShadow.Image = "rbxassetid://6015897843"
        fabShadow.ImageColor3 = Colors.Accent
        fabShadow.ImageTransparency = 0.5
        fabShadow.ScaleType = Enum.ScaleType.Slice
        fabShadow.SliceCenter = Rect.new(49, 49, 450, 450)
        fabShadow.ZIndex = 99
        ...
        fab.MouseButton1Click:Connect(function()
            ...
            -- Pulse animation
            pcall(function()
                BW.TweenService:Create(fab, TweenInfo.new(0.15, Enum.EasingStyle.Back), {
                    Size = UDim2.new(0, 48 * scale, 0, 48 * scale)
                }):Play()
                task.delay(0.15, function()
                    pcall(function()
                        BW.TweenService:Create(fab, TweenInfo.new(0.15, Enum.EasingStyle.Back), {
                            Size = UDim2.new(0, 56 * scale, 0, 56 * scale)
                        }):Play()
                    end)
                end)
            end)
                ...
                if math.abs(delta) > swipeThreshold then
                    ...
                    if #tabNames > 1 then
                        ...
                        for i, name in ipairs(tabNames) do
                            if name == self.CurrentPage then currentIdx = i; break end
                        end
                        local newIdx
                        if delta > 0 then
                            newIdx = math.max(1, currentIdx - 1)
                        else
                            newIdx = math.min(#tabNames, currentIdx + 1)
                        end
                        -- Simulate tab click
                        local btn = self.TabButtons[tabNames[newIdx]]
...
function Library:Tab(name, icon)
    ...
    btn.TextSize = ifelse(BW.isMobile, math.floor(10 * scale), 12)
    ...
    pf.ScrollBarThickness = ifelse(BW.isMobile, 2, 3)
    ...
    listLayout.Padding = UDim.new(0, ifelse(BW.isMobile, 3, 4))
    ...
    function page:Toggle(name, default, callback, category, tooltip)
        ...
        f.Size = UDim2.new(1, 0, 0, ifelse(BW.isMobile, 44, 36))
        ...
        lbl.TextSize = ifelse(BW.isMobile, 13, 12)
        lbl.Font = Enum.Font.GothamMedium
        lbl.TextXAlignment = Enum.TextXAlignment.Left
        lbl.Parent = f

        -- Toggle Track (bigger on mobile for easy tapping)
        local trackW = ifelse(BW.isMobile, 44, 36)
        local trackH = ifelse(BW.isMobile, 24, 20)
        ...
        -- Toggle Knob
        local knobSize = ifelse(BW.isMobile, 20, 16)
        local knob = Instance.new("Frame")
        knob.Size = UDim2.new(0, knobSize, 0, knobSize)
        knob.Position = ifelse(cfg.Default, UDim2.new(1, -(knobSize + 2), 0.5, -(knobSize / 2)), UDim2.new(0, 2, 0.5, -(knobSize / 2)))
        ...
        local function doToggle()
            ...
            local newPos = ifelse(on, UDim2.new(1, -(knobSize + 2), 0.5, -(knobSize / 2)), UDim2.new(0, 2, 0.5, -(knobSize / 2)))
            ...
            pcall(function() if cfg.Callback then cfg.Callback(on) end end)
        end

        track.MouseButton1Click:Connect(doToggle)

        -- Mobile: also support tapping the entire row
        if BW.isMobile then
    ...
    function page:Slider(name, min, max, default, callback, category)
        ...
        local f = Instance.new("Frame")
        f.Size = UDim2.new(1, 0, 0, ifelse(BW.isMobile, 56, 48))
        f.BackgroundColor3 = Colors.Card
        f.BorderSizePixel = 0
        f.LayoutOrder = #pf:GetChildren()
        f.Parent = pf
        ...
        -- Slider Track (bigger touch target on mobile)
        local trackH = ifelse(BW.isMobile, 10, 6)
        local trackY = ifelse(BW.isMobile, 36, 32)
        ...
        local knobSize = ifelse(BW.isMobile, 18, 14)
        local knob = Instance.new("Frame")
        knob.Size = UDim2.new(0, knobSize, 0, knobSize)
        knob.Position = UDim2.new((def - min) / (max - min), -(knobSize / 2), 0.5, -(knobSize / 2))
        knob.BackgroundColor3 = Color3.new(1, 1, 1)
        knob.BorderSizePixel = 0
        knob.ZIndex = 2
        knob.Parent = track
        ...
        -- Mobile: bigger invisible touch area around slider
        if BW.isMobile then
            local touchArea = Instance.new("TextButton")
            touchArea.Size = UDim2.new(1, 0, 0, 40)
            touchArea.Position = UDim2.new(0, 0, 0, trackY - 12)
        ...
        BW.UserInputService.InputChanged:Connect(function(input)
            if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                ...
                pcall(function() if cfg.Callback then cfg.Callback(v) end end)
            end
        end)
    end
    ...
    function page:Dropdown(cfg)
        ...
        local itemH = ifelse(BW.isMobile, 34, 28)
        ...
        local lbl = Instance.new("TextLabel")
        lbl.Size = UDim2.new(0.5, 0, 0, ifelse(BW.isMobile, 44, 36))
        lbl.Position = UDim2.new(0, 12, 0, 0)
        lbl.BackgroundTransparency = 1
        lbl.Text = cfg.Name
        lbl.TextColor3 = Colors.Text
        ...
        sel.Size = UDim2.new(0.45, -8, 0, ifelse(BW.isMobile, 30, 26))
        sel.Position = UDim2.new(0.55, 0, 0, ifelse(BW.isMobile, 7, 5))
        sel.BackgroundColor3 = Colors.CardHover
        sel.BorderSizePixel = 0
        sel.Text = "  " .. defaultVal .. " ▾"
        sel.TextColor3 = Colors.TextDim
        sel.TextSize = ifelse(BW.isMobile, 12, 11)
        ...
        sel.MouseButton1Click:Connect(function()
            open = not open
            f.Size = UDim2.new(1, 0, 0, ifelse(open, ifelse(BW.isMobile, 44, 36) + #cfg.Options * itemH + 4, ifelse(BW.isMobile, 44, 36)))
        end)

        for i, opt in ipairs(cfg.Options) do
            local ob = Instance.new("TextButton")
            ob.Size = UDim2.new(1, 0, 0, itemH)
            ob.Position = UDim2.new(0, 0, 0, ifelse(BW.isMobile, 44, 36) + (i - 1) * itemH)
            ob.BackgroundColor3 = Colors.Card
            ob.BorderSizePixel = 0
            ob.Text = "    " .. opt
            ob.TextColor3 = Colors.TextDim
            ob.TextSize = ifelse(BW.isMobile, 12, 11)
    ...
    function page:Button(cfg)
        ...
        b.Size = UDim2.new(1, 0, 0, ifelse(BW.isMobile, 40, 32))
        b.BackgroundColor3 = ifelse(cfg.Color, cfg.Color, Colors.AccentDark)
        b.BorderSizePixel = 0
        b.Text = cfg.Name
        b.TextColor3 = Color3.new(1, 1, 1)
        b.TextSize = ifelse(BW.isMobile, 14, 12)
    ...
    function page:Label(text)
        ...
        l.Size = UDim2.new(1, 0, 0, ifelse(BW.isMobile, 24, 20))
        l.BackgroundTransparency = 1
        l.Text = "  " .. text
        l.TextColor3 = Colors.TextMuted
        l.TextSize = ifelse(BW.isMobile, 11, 10)
    ...
    function page:Input(cfg)
        ...
        tb.Size = UDim2.new(0.55, -8, 0, ifelse(BW.isMobile, 30, 24))
        tb.Position = UDim2.new(0.45, 0, 0.5, ifelse(BW.isMobile, -15, -12))
        ...
        tb.TextSize = ifelse(BW.isMobile, 12, 11)
...
BW.Library = Library
print("[UI] Library loaded v6.0 (Mobile + PC)")
return Library
...
page:Slider("Jump Boost Power", 1, 10, 2, function(v) Flags.JumpPow = v end)
task.spawn(function()
    while task.wait(0.1) do
