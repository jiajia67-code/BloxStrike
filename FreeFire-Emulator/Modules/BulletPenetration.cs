...
namespace FreeFire_Emulator.Modules
{
    internal static class BulletPenetration
    {
        ...
        private static readonly byte[] Search = new byte[]
        {
            ...
            0x00, 0x10, 0x90, 0xE5, 0x04, 0x20, 0x90, 0xE5,
        ...
        public static void FindPattern(Memory mem)
        {
            ...
            long[] r = mem.FindPatternAll(Search);
            if (r != null && r.Length > 0) FoundAddresses.AddRange(r);
        ...
        public static bool ApplyPatch(Memory mem)
        {
            ...
            Console.WriteLine($"[BulletPenetration] Applied {c}/{FoundAddresses.Count}");
            return c > 0;
        }
